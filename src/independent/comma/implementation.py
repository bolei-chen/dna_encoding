"""
Comma code implementation (Smith et al., as summarized in the survey).

Codeword form:
  G + p1 p2 p3 p4 p5
where each pi is in {A, C, T} and exactly two of the five positions are C.
This yields 80 length-6 symbols.

To fit the repo's 8-bit character workflow, one byte is represented as two
comma symbols (total 12 nt) via a reversible mixed-radix map.
"""
...
class CommaCodeGenerator:
    """
    Comma-code generator with deterministic ranking/unranking.
    """

    comma_base: str = "G"
    payload_alphabet: tuple[str, ...] = ("A", "C", "T")
    symbol_length: int = 6
    ...
    def _build_symbol_table(self) -> List[str]:
        out: List[str] = []
        for payload_tuple in itertools.product(self.payload_alphabet, repeat=5):
            payload = "".join(payload_tuple)
            if payload.count("C") != 2:
                continue
            out.append(f"{self.comma_base}{payload}")
        return sorted(out)
    ...
    def decode_symbol(self, codeword: str) -> int:
        if codeword not in self._symbol_to_index:
            raise ValueError(f"invalid comma code symbol: {codeword}")
    ...
    def encode_byte(self, byte_value: int | str) -> str:
        ...
        # Mixed-radix representation with 4 x 80 = 320 states.
    ...
    def decode_byte(self, encoded: str, *, bit_length: int = 8) -> str:
        ...
        if value >= 256:
            raise ValueError("decoded mixed-radix value does not map to one byte")
